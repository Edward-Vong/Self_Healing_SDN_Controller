...
if run "$ATTACKER_HOST" "sudo -n arping -I $ATTACK_IFACE_VAL -c 2 $ATTACK_TARGET >/tmp/arp_demo_attack.log 2>&1" && run "$ATTACKER_HOST" "grep -Eq '([1-9][0-9]*) packets received|Received [1-9]' /tmp/arp_demo_attack.log"; then
...
if run "$TRUSTED_HOST" "sudo -n arping -I $TRUSTED_VALID_IFACE_VAL -c 2 $VALID_TARGET >/tmp/arp_demo_valid.log 2>&1" && run "$TRUSTED_HOST" "grep -Eq '([1-9][0-9]*) packets received|Received [1-9]' /tmp/arp_demo_valid.log"; then
